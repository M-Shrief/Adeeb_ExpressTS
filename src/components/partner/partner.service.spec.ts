...
describe.concurrent('Testing PartnerService', async () => {
    ...
    describe("Testing signup()", async() => {
        ...
        test("SignUp successfully after validation and hashing password", async() => {
            vi.spyOn(PartnerDB, "signup").mockResolvedValue({name, phone, password: hashed} as PartnerType);
            const result = await PartnerService.signup({name, phone, password} as PartnerType)
            expect(result).toStrictEqual({name, phone, password: hashed} as PartnerType)
        })
        test("Returns false after inValid data", async() => {
            vi.spyOn(PartnerDB, "signup").mockResolvedValue({name, phone, password: hashed} as PartnerType);
            
            const result1 = await PartnerService.signup({name, phone} as PartnerType)
            expect(result1).toEqual(false)
            const result2 = await PartnerService.signup({name, password} as PartnerType)
            expect(result2).toEqual(false)
            const result3 = await PartnerService.signup({phone, password} as PartnerType)
    ...
    describe("Testing login()", async() => {
        let phone = "01235554567",
            ...
            hashed = await hashPassword(password);
        test("SignUp successfully after validation and hashing password", async() => {
            vi.spyOn(PartnerDB, "login").mockResolvedValue({ phone, password: hashed} as PartnerType);
        ...
        test("Returns false after for non-existing phone or inValid password", async() => {
            ...
            vi.spyOn(PartnerDB, "login").mockResolvedValue({ phone, password} as PartnerType);
            const result = await PartnerService.login(phone, password)
            expect(result).toEqual(false)
        })
    })
    ...
    describe("Testing update()", async() => {
        ...
        test("Updated Partner data successfully after validation and hashing password", async() => {
            ...
            const result2 = await PartnerService.update("1", {phone} as PartnerType)
            expect(result2).toStrictEqual(partner)
            const result3 = await PartnerService.update("1", {password} as PartnerType)
        ...
        test("Returns false after inValid data", async() => {
            vi.spyOn(PartnerDB, "update").mockResolvedValue(partner);
              ...
              const result1 = await PartnerService.update("1", {name: "sd"} as PartnerType)
              expect(result1).toEqual(false)
              const result2 = await PartnerService.update("1", {phone: "sd"} as PartnerType)
              expect(result2).toEqual(false)
              const result3 = await PartnerService.update("1", {password: "sd"} as PartnerType)
              expect(result3).toEqual(false)
        })
    })
